...
void main() {
  ...
  setUp(() {
    ...
    when(mockAdapter.scanResults).thenAnswer((_) => scanResultsController.stream);
  ...
  group('BleNirScanService', () {
    group('connection state stream', () {
      ...
      test('emits connecting then connected on successful connect', () async {
        ...
        when(mockService.uuid).thenReturn(Guid('0000180a-0000-1000-8000-00805f9b34fb'));
    ...
    group('device discovery', () {
      test('filters devices by NIRScan name prefix', () async {
        ...
        final subscription = service.discoveredDevices.listen(discoveredDevices.add);

        await service.startDeviceScan();

        // Emit scan results - mix of NIRScan and other devices
        final nirDevice = _createMockScanResult('NIRScan Nano ABC', 'AA:BB:CC:DD:EE:FF', -50);
        final otherDevice = _createMockScanResult('Random Device', '11:22:33:44:55:66', -60);
        final anotherNir = _createMockScanResult('NIRScan Nano XYZ', '11:22:33:44:55:77', -70);
      ...
      test('does not emit devices without NIRScan prefix', () async {
        ...
        final otherDevice = _createMockScanResult('Other BLE Device', '11:22:33:44:55:66', -60);
      ...
      test('includes RSSI in discovered device', () async {
        ...
        final nirDevice = _createMockScanResult('NIRScan Nano', 'AA:BB:CC:DD:EE:FF', -55);
    ...
    group('disconnect cleanup', () {
      ...
      test('cancels notification subscriptions on disconnect', () async {
        ...
        when(mockCharacteristic.uuid).thenReturn(Guid('00002a29-0000-1000-8000-00805f9b34fb'));
    ...
    group('getDeviceInfo', () {
      ...
      test('reads all DIS characteristics and returns DeviceInfo', () async {
        ...
        when(mockDevice.discoverServices()).thenAnswer((_) async => [mockDisService]);
        when(mockDevice.connectionState).thenAnswer(
          (_) => Stream.value(BluetoothConnectionState.connected),
        );
        when(mockDevice.disconnect()).thenAnswer((_) async {});
        ...
        // DIS Service UUID
        when(mockDisService.uuid).thenReturn(Guid('0000180a-0000-1000-8000-00805f9b34fb'));
        ...
        when(manufNameChar.uuid).thenReturn(Guid('00002a29-0000-1000-8000-00805f9b34fb'));
        when(manufNameChar.read()).thenAnswer((_) async => 'Texas Instruments'.codeUnits);

        when(modelNumberChar.uuid).thenReturn(Guid('00002a24-0000-1000-8000-00805f9b34fb'));
        when(modelNumberChar.read()).thenAnswer((_) async => 'DLP NIRscan Nano'.codeUnits);

        when(serialNumberChar.uuid).thenReturn(Guid('00002a25-0000-1000-8000-00805f9b34fb'));
        when(serialNumberChar.read()).thenAnswer((_) async => 'SN12345'.codeUnits);

        when(hwRevChar.uuid).thenReturn(Guid('00002a27-0000-1000-8000-00805f9b34fb'));
        when(hwRevChar.read()).thenAnswer((_) async => '2.0.0'.codeUnits);

        when(tivaFwRevChar.uuid).thenReturn(Guid('00002a26-0000-1000-8000-00805f9b34fb'));
        when(tivaFwRevChar.read()).thenAnswer((_) async => '2.4.4'.codeUnits);

        when(speccRevChar.uuid).thenReturn(Guid('00002a28-0000-1000-8000-00805f9b34fb'));
    ...
    group('getDeviceStatus', () {
      ...
      test('reads all status characteristics and returns DeviceStatus', () async {
        final mockDevice = MockBluetoothDevice();
        ...
        when(mockDevice.discoverServices()).thenAnswer((_) async => [mockBasService, mockGgisService]);
        when(mockDevice.connectionState).thenAnswer(
          (_) => Stream.value(BluetoothConnectionState.connected),
        );
        when(mockDevice.disconnect()).thenAnswer((_) async {});
        ...
        when(mockBasService.uuid).thenReturn(Guid('0000180f-0000-1000-8000-00805f9b34fb'));
        when(mockBasService.characteristics).thenReturn([battLvlChar]);

        // GGIS Service UUID
        when(mockGgisService.uuid).thenReturn(Guid('53455201-444c-5020-4e49-52204e616e6f'));
        ...
        when(battLvlChar.uuid).thenReturn(Guid('00002a19-0000-1000-8000-00805f9b34fb'));
        when(battLvlChar.read()).thenAnswer((_) async => [75]);

        // Temperature - int16 (2550 = 25.50°C)
        when(tempChar.uuid).thenReturn(Guid('43484101-444c-5020-4e49-52204e616e6f'));
        when(tempChar.read()).thenAnswer((_) async => [0xF6, 0x09]); // 2550 little-endian

        // Humidity - uint16 (4520 = 45.20%)
        when(humidChar.uuid).thenReturn(Guid('43484102-444c-5020-4e49-52204e616e6f'));
        when(humidChar.read()).thenAnswer((_) async => [0xA8, 0x11]); // 4520 little-endian

        // Device Status - uint16 (0x0001)
        when(devStatusChar.uuid).thenReturn(Guid('43484103-444c-5020-4e49-52204e616e6f'));
        when(devStatusChar.read()).thenAnswer((_) async => [0x01, 0x00]);

        // Error Status - uint16 (0x0000 = no errors)
        when(errStatusChar.uuid).thenReturn(Guid('43484104-444c-5020-4e49-52204e616e6f'));
      ...
      test('handles error status flags', () async {
        ...
        when(mockGgisService.uuid).thenReturn(Guid('53455201-444c-5020-4e49-52204e616e6f'));
        when(mockGgisService.characteristics).thenReturn([
          tempChar, humidChar, devStatusChar, errStatusChar,
        ]);
      ...
      test('handles negative temperature values', () async {
        ...
        when(tempChar.uuid).thenReturn(Guid('43484101-444c-5020-4e49-52204e616e6f'));
        when(tempChar.read()).thenAnswer((_) async => [0x0C, 0xFE]); // -500 little-endian
    ...
    group('notification subscriptions', () {
      test('subscribeToNotifications enables notifications on characteristic', () async {
        final mockDevice = MockBluetoothDevice();
        ...
        // GCIS Service
        when(mockGcisService.uuid).thenReturn(Guid('53455204-444c-5020-4e49-52204e616e6f'));
        when(mockGcisService.characteristics).thenReturn([mockRetSpecCalCoeffChar]);

        // Notification characteristic
        when(mockRetSpecCalCoeffChar.uuid).thenReturn(Guid('4348410e-444c-5020-4e49-52204e616e6f'));
        when(mockRetSpecCalCoeffChar.setNotifyValue(true)).thenAnswer((_) async => true);
        when(mockRetSpecCalCoeffChar.onValueReceived).thenAnswer((_) => notificationController.stream);
      ...
      test('subscribeToNotifications throws when not connected', () {
        expect(
          () => service.subscribeToNotifications(Guid('4348410e-444c-5020-4e49-52204e616e6f')),
          throwsA(isA<NotConnectedException>()),
        );
      });

      test('subscribeToNotifications throws when characteristic not found', () async {
        final mockDevice = MockBluetoothDevice();
        final mockDisService = MockBluetoothService();
      ...
      test('subscribeToAllNotifications subscribes to all notification characteristics', () async {
        final mockDevice = MockBluetoothDevice();
        final mockGcisService = MockBluetoothService();
        ...
        mockChars.addAll([gcisRetSpecCalCoeff, gcisRetRefCalCoeff, gcisRetRefCalMatrix]);
        ...
        mockChars.addAll([
          gsdisStartScan, gsdisRetScanName, gsdisRetScanType, gsdisRetScanDate,
          gsdisRetPktFmtVer, gsdisRetSerScanDataStruct, gsdisSdStoredScanIndListData, gsdisClearScan,
        ]);
        ...
        when(mockDevice.discoverServices()).thenAnswer((_) async => [mockGcisService, mockGsdisService, mockGscisService]);
        when(mockDevice.connectionState).thenAnswer(
          (_) => Stream.value(BluetoothConnectionState.connected),
        );
        when(mockDevice.disconnect()).thenAnswer((_) async {});
        ...
        // GCIS Service
        when(mockGcisService.uuid).thenReturn(Guid('53455204-444c-5020-4e49-52204e616e6f'));
        when(mockGcisService.characteristics).thenReturn([gcisRetSpecCalCoeff, gcisRetRefCalCoeff, gcisRetRefCalMatrix]);

        // GSDIS Service
        when(mockGsdisService.uuid).thenReturn(Guid('53455206-444c-5020-4e49-52204e616e6f'));
        ...
        // GSCIS Service
        when(mockGscisService.uuid).thenReturn(Guid('53455205-444c-5020-4e49-52204e616e6f'));
        when(mockGscisService.characteristics).thenReturn([gscisRetStoredConfList, gscisRetScanConfData]);

        // Setup notification characteristic UUIDs and behaviors
        when(gcisRetSpecCalCoeff.uuid).thenReturn(Guid('4348410e-444c-5020-4e49-52204e616e6f'));
        when(gcisRetRefCalCoeff.uuid).thenReturn(Guid('43484110-444c-5020-4e49-52204e616e6f'));
        when(gcisRetRefCalMatrix.uuid).thenReturn(Guid('43484112-444c-5020-4e49-52204e616e6f'));
        when(gsdisStartScan.uuid).thenReturn(Guid('4348411d-444c-5020-4e49-52204e616e6f'));
        when(gsdisRetScanName.uuid).thenReturn(Guid('43484120-444c-5020-4e49-52204e616e6f'));
        when(gsdisRetScanType.uuid).thenReturn(Guid('43484122-444c-5020-4e49-52204e616e6f'));
        when(gsdisRetScanDate.uuid).thenReturn(Guid('43484124-444c-5020-4e49-52204e616e6f'));
        when(gsdisRetPktFmtVer.uuid).thenReturn(Guid('43484126-444c-5020-4e49-52204e616e6f'));
        when(gsdisRetSerScanDataStruct.uuid).thenReturn(Guid('43484128-444c-5020-4e49-52204e616e6f'));
        when(gsdisSdStoredScanIndListData.uuid).thenReturn(Guid('4348411b-444c-5020-4e49-52204e616e6f'));
        when(gsdisClearScan.uuid).thenReturn(Guid('4348411e-444c-5020-4e49-52204e616e6f'));
        when(gscisRetStoredConfList.uuid).thenReturn(Guid('43484115-444c-5020-4e49-52204e616e6f'));
        when(gscisRetScanConfData.uuid).thenReturn(Guid('43484117-444c-5020-4e49-52204e616e6f'));

        for (final char in mockChars) {
          when(char.setNotifyValue(true)).thenAnswer((_) async => true);
          when(char.onValueReceived).thenAnswer((_) => notificationController.stream);
        }
    ...
    group('performScan', () {
      ...
      test('writes save flag 0x00 to start scan characteristic', () async {
        ...
        final startScanNotifyController = StreamController<List<int>>.broadcast();
        ...
        when(mockDevice.discoverServices()).thenAnswer((_) async => [mockGsdisService]);
        when(mockDevice.connectionState).thenAnswer(
          (_) => Stream.value(BluetoothConnectionState.connected),
        );
        when(mockDevice.disconnect()).thenAnswer((_) async {});
        ...
        when(mockStartScanChar.uuid).thenReturn(Guid('4348411d-444c-5020-4e49-52204e616e6f'));
        when(mockStartScanChar.write(any, withoutResponse: anyNamed('withoutResponse')))
            .thenAnswer((_) async {});
        when(mockStartScanChar.setNotifyValue(true)).thenAnswer((_) async => true);
        when(mockStartScanChar.onValueReceived).thenAnswer((_) => startScanNotifyController.stream);
        ...
        verify(mockStartScanChar.write([0x00], withoutResponse: anyNamed('withoutResponse'))).called(1);
      ...
      test('writes save flag 0x01 when saveToSd is true', () async {
        ...
        verify(mockStartScanChar.write([0x01], withoutResponse: anyNamed('withoutResponse'))).called(1);
      ...
      test('throws ScanFailedException when scan notification is not 0xFF', () async {
        final mockDevice = MockBluetoothDevice();
        final mockGsdisService = MockBluetoothService();
        final mockStartScanChar = MockBluetoothCharacteristic();
        final startScanNotifyController = StreamController<List<int>>.broadcast();
      ...
      test('returns ScanData on successful scan', () async {
        ...
        final startScanNotifyController = StreamController<List<int>>.broadcast();
        final retScanNameNotifyController = StreamController<List<int>>.broadcast();
        final retScanTypeNotifyController = StreamController<List<int>>.broadcast();
        final retScanDateNotifyController = StreamController<List<int>>.broadcast();
        final retPktFmtVerNotifyController = StreamController<List<int>>.broadcast();
        final retSerScanDataNotifyController = StreamController<List<int>>.broadcast();
        ...
        // GSDIS_REQ_SCAN_NAME
        when(mockReqScanNameChar.uuid).thenReturn(Guid('4348411f-444c-5020-4e49-52204e616e6f'));
        when(mockReqScanNameChar.write(any, withoutResponse: anyNamed('withoutResponse')))
            .thenAnswer((_) async {});

        // GSDIS_RET_SCAN_NAME (notify)
        when(mockRetScanNameChar.uuid).thenReturn(Guid('43484120-444c-5020-4e49-52204e616e6f'));
        when(mockRetScanNameChar.setNotifyValue(true)).thenAnswer((_) async => true);
        when(mockRetScanNameChar.onValueReceived).thenAnswer((_) => retScanNameNotifyController.stream);

        // GSDIS_REQ_SCAN_TYPE
        when(mockReqScanTypeChar.uuid).thenReturn(Guid('43484121-444c-5020-4e49-52204e616e6f'));
        when(mockReqScanTypeChar.write(any, withoutResponse: anyNamed('withoutResponse')))
            .thenAnswer((_) async {});

        // GSDIS_RET_SCAN_TYPE (notify)
        when(mockRetScanTypeChar.uuid).thenReturn(Guid('43484122-444c-5020-4e49-52204e616e6f'));
        when(mockRetScanTypeChar.setNotifyValue(true)).thenAnswer((_) async => true);
        when(mockRetScanTypeChar.onValueReceived).thenAnswer((_) => retScanTypeNotifyController.stream);

        // GSDIS_REQ_SCAN_DATE
        when(mockReqScanDateChar.uuid).thenReturn(Guid('43484123-444c-5020-4e49-52204e616e6f'));
        when(mockReqScanDateChar.write(any, withoutResponse: anyNamed('withoutResponse')))
            .thenAnswer((_) async {});

        // GSDIS_RET_SCAN_DATE (notify)
        when(mockRetScanDateChar.uuid).thenReturn(Guid('43484124-444c-5020-4e49-52204e616e6f'));
        when(mockRetScanDateChar.setNotifyValue(true)).thenAnswer((_) async => true);
        when(mockRetScanDateChar.onValueReceived).thenAnswer((_) => retScanDateNotifyController.stream);

        // GSDIS_REQ_PKT_FMT_VER
        when(mockReqPktFmtVerChar.uuid).thenReturn(Guid('43484125-444c-5020-4e49-52204e616e6f'));
        when(mockReqPktFmtVerChar.write(any, withoutResponse: anyNamed('withoutResponse')))
            .thenAnswer((_) async {});

        // GSDIS_RET_PKT_FMT_VER (notify)
        when(mockRetPktFmtVerChar.uuid).thenReturn(Guid('43484126-444c-5020-4e49-52204e616e6f'));
        when(mockRetPktFmtVerChar.setNotifyValue(true)).thenAnswer((_) async => true);
        when(mockRetPktFmtVerChar.onValueReceived).thenAnswer((_) => retPktFmtVerNotifyController.stream);

        // GSDIS_REQ_SER_SCAN_DATA_STRUCT
        when(mockReqSerScanDataChar.uuid).thenReturn(Guid('43484127-444c-5020-4e49-52204e616e6f'));
        when(mockReqSerScanDataChar.write(any, withoutResponse: anyNamed('withoutResponse')))
            .thenAnswer((_) async {});

        // GSDIS_RET_SER_SCAN_DATA_STRUCT (notify - multi-packet)
        when(mockRetSerScanDataChar.uuid).thenReturn(Guid('43484128-444c-5020-4e49-52204e616e6f'));
        when(mockRetSerScanDataChar.setNotifyValue(true)).thenAnswer((_) async => true);
        when(mockRetSerScanDataChar.onValueReceived).thenAnswer((_) => retSerScanDataNotifyController.stream);
        ...
        retSerScanDataNotifyController.add([0x01, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE]);
    ...
    group('syncTime', () {
      ...
      test('writes formatted time to GDTS characteristic', () async {
        ...
        when(mockDevice.discoverServices()).thenAnswer((_) async => [mockGdtsService]);
        when(mockDevice.connectionState).thenAnswer(
          (_) => Stream.value(BluetoothConnectionState.connected),
        );
        when(mockDevice.disconnect()).thenAnswer((_) async {});
        ...
        // GDTS Service
        when(mockGdtsService.uuid).thenReturn(Guid('53455203-444c-5020-4e49-52204e616e6f'));
        when(mockGdtsService.characteristics).thenReturn([mockTimeChar]);

        // GDTS Time characteristic
        when(mockTimeChar.uuid).thenReturn(Guid('4348410c-444c-5020-4e49-52204e616e6f'));
